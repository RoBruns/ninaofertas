...
type PlatformNeeds = {
  ...
  credential: { kind: string; label: string; hint: string }
}

// O que o worker lê de cada plataforma (core/platforms): config vai em claro,
// credential vai cifrada e nunca volta para o dashboard.
const PLATFORM_NEEDS: Readonly<Record<string, PlatformNeeds>> = {
  shopee: {
    config: [{ key: 'app_id', label: 'App ID', hint: 'Painel de afiliados da Shopee → Open API.' }],
    credential: { kind: 'app_secret', label: 'App Secret', hint: 'Na mesma tela do App ID.' },
  },
  mercadolivre: {
...
const FALLBACK_KINDS = [{ kind: 'cookie', label: 'Cookie' }, { kind: 'token', label: 'Token' }]
...
function AccountForm({ platforms, account, onClose }: { platforms: Platform[]; account?: Account; onClose: () => void }) {
  ...
  const slug = account?.platform.slug ?? platforms.find((platform) => platform.id === platformId)?.slug ?? ''
  const needs = PLATFORM_NEEDS[slug]
  const requiredKeys = new Set(needs?.config.map((item) => item.key) ?? [])
  const configValue = (key: string) => configRows.find((row) => row.key === key)?.value ?? ''
  const setConfigValue = (key: string, value: string) => setConfigRows((rows) => rows.some((row) => row.key === key) ? rows.map((row) => row.key === key ? { ...row, value } : row) : [...rows, { key, value }])
...
function CredentialRenewal({ account, kind, newCredential = account.credentials.length === 0, onClose }: { account: Account; kind: string; newCredential?: boolean; onClose: () => void }) {
  ...
  const needs = PLATFORM_NEEDS[account.platform.slug]
  const kinds = needs ? [needs.credential] : FALLBACK_KINDS
  const kindLabel = kinds.find((item) => item.kind === credentialKind)?.label ?? credentialKind
  ...
  return <Modal title={`${newCredential ? 'Cadastrar' : 'Renovar'} ${kindLabel} — ${account.label}`} onClose={onClose}><form className="form-grid" onSubmit={(event) => void submit(event)}><p className="security-note">O valor é enviado uma única vez e não será exibido nem guardado pelo dashboard.</p>{newCredential && kinds.length > 1 && <Field label="Tipo de credencial"><select value={credentialKind} onChange={(event) => setCredentialKind(event.target.value)}>{kinds.map((item) => <option value={item.kind} key={item.kind}>{item.label}</option>)}</select></Field>}{needs && <p className="muted">{needs.credential.hint}</p>}<Field label={kindLabel}><textarea aria-label={kindLabel} required autoComplete="off" spellCheck={false} value={value} onChange={(event) => setValue(event.target.value)} /></Field>{pending && <p>Salvando e testando…</p>}{result && <div className={result.ok ? 'inline-success' : 'inline-error'} role="status"><strong>{result.ok ? 'Funcionou' : 'Não funcionou'}</strong><span>{result.message}</span></div>}<div className="form-actions"><button className="button ghost" type="button" onClick={onClose}>Fechar</button><button className="button primary" disabled={pending || !value.trim()}>Salvar e testar</button></div></form></Modal>
}
