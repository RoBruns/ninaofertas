...
import { formatBRL, formatCount, formatPercent, formatRatio, healthText, isNegativeMoney } from '../lib/format'
...
export function Performance() {
  ...
  return <section><div className="page-heading"><div><h1>Desempenho</h1><p>Descubra o que performa e onde o investimento não volta.</p></div><label className="filter-field"><span>Dimensão</span><select value={dimension} onChange={(event) => setDimension(event.target.value as MetricDimension)}>{Object.entries(dimensionLabels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label></div><GlobalFilters />{query.isPending ? <Spinner label="Carregando comparativo" /> : <><DataWarnings warnings={query.data?.warnings ?? []} /><div className="responsive-table performance-table"><table><thead><tr><th><button onClick={() => chooseSort('name')}>{dimensionLabels[dimension]}</button></th>{dimension === 'bot' && <th>Saúde</th>}{columns.map(([key, label]) => <th key={key}><button onClick={() => chooseSort(key)}>{label}{sort === key ? descending ? ' ↓' : ' ↑' : ''}</button></th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td data-label={dimensionLabels[dimension]}>{item.name}</td>{dimension === 'bot' && <td data-label="Saúde">{typeof item.id === 'string' ? healthText(botHealth.data?.[item.id]?.status, botHealth.data?.[item.id]?.message) : healthText(undefined, undefined)}</td>}{columns.map(([key, label, render]) => <td key={key} data-label={label} className={key === 'profit' && isNegativeMoney(item.profit) ? 'negative-money' : ''}>{render(item)}</td>)}</tr>)}</tbody></table></div></>}</section>
}
